<?php
$session = $this->request->session()->read("adminlogin")["supplier_nrint"] == "" || $this->request->session()->read("adminlogin")["supplier_nrint"] == "C86OEET2" ? $this->request->session()->read("waitinglist") : $this->request->session()->read("adminlogin")["supplier_nrint"];
?>
...
<div class="box box-default">
    ...
    <div class="box-header">
        <h3 class="box-title"><i class="fa fa-cog"></i> <?= __('Orders naar Nederland') ?></h3>
    </div>
    ...
    <div class="box-body">
            ...
            <ul class="nav nav-tabs">
                <?php if($session == ""){ ?>
                <li class="active"><a href="#tab_1-1" data-toggle="tab"><?= __('Overzicht') ?> (<?= $this->Subcount->makeSubCount($aOrders, "to_nl", "0", "0") ?>)</a></li>
                ...
                <?php if($this->request->session()->read("adminlogin")["supplier_nrint"] == ""){ ?>
                    <div class="pull-right">
                        <form method="post" action="/Transport/setSession/waitinglist/">
            ...
            <div class="tab-content">
                        ...
                        <?php }else{ ?>
                            ...
                            <table class="table table-condensed table-hover" id="table2excel">
                                ...
                                <tr>
                                    ...
                                    <th width="200" class="text-right"><?= __('In de wacht') ?></th>
                                </tr>
                                </thead>
                                ...
                                foreach($aOrders as $aOrder){
                                    ...
                                    ?>
                                        <tr data-touch-href="/Order/<?= $aOrder->HdIntPrplPartialDelivery->order_nrint ?>" <?php if($aOrder->HdIntPrplPartialDelivery->HdIntPrplViewItem->HdIntPrplGeneral->fsc_order == 1){ echo "class='success'"; } ?>>
                ...
                <?php foreach($aCompany as $company){ ?>
                    ...
                    <div class="tab-pane active" id="<?= $company["BKHADNRINT"] ?>">
                        <form method="post" action="/Transport/setSendDate/">
                        ...
                        <?php }else{ ?>
                                    ...
                                    if($aOrder->HdIntPrplPackaging->adres_nrint == $session){
                                        ...
                                        <tr data-touch-href="/Order/<?= $aOrder->HdIntPrplPartialDelivery->HdIntPrplViewItem->orderrule_id ?>">
                                            ...
                                            <td class="text-right">
                                                ...
                                                <input style="width:100%;text-align:right;" value="" type="number" name="pallets[<?= $aOrder->id ?>]" />
                                            </td>
                                        </tr>
                                        <?php $total += $newQty; ?>
                                    <?php }} ?>
                            ...
                            <div class="pull-right">
                                <button type="submit" name="submit" value="saveAndShowBatch" class="btn btn-sm bg-purple"><i class="fa fa-file-o"></i> <?= __('Maak verzendlijst') ?></button>
                            </div>
