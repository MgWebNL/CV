...
<div class="box box-default">
    ...
    <div class="box-body">
            ...
            <section class="invoice">
                <form method="post" action="/Prepackaging/sendOrdersToPackaging/">
                    <!-- title row -->
                    <div class="row">
                        <div class="col-xs-12">
                            <h2 class="page-header">
                                <?= __('Ontvangstbevestiging') ?>  <?= $aProforma["Printer"]["HdIntPrplCompanies"]["adres_alias"] ?>
                                <small class="pull-right"><?= __('Datum') ?>: <?= date('d-m-Y') ?></small>
                            </h2>
                            <input type="hidden" name="printer_nrint" value="<?= $aProforma["Printer"]["BKHADNRINT"] ?>" />
                            <input type="hidden" name="printer_name" value="<?= $aProforma["Printer"]["HdIntPrplCompanies"]["adres_alias"] ?>" />
                        </div><!-- /.col -->
                    </div>
                    <!-- info row -->
                    ...
                    <div class="row no-print">
                        <div class="col-xs-12">
                            <button onclick="javascript:history.go(-1);" class="btn btn-default btn-sm"><i class="fa fa-share fa-flip-horizontal"></i> <?= __('Ontvangstbevestiging aanpassen') ?></button>
                            <button type="submit" name="submit" value="ok_no_send" class="hidden btn bg-purple btn-sm pull-right" style="margin-right: 5px;"><i class="fa fa-check"></i> <?= __('Bevestigen') ?></button>
                        </div>
                    </div>
                </form>
            </section><!-- /.content -->
