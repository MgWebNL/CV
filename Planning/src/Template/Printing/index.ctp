<?php
    $session = $this->request->session()->read("adminlogin")["supplier_nrint"] == "" || $this->request->session()->read("adminlogin")["supplier_nrint"] == "C86OEET2" ? $this->request->session()->read("printer") : $this->request->session()->read("adminlogin")["supplier_nrint"];
    use Cake\I18n\Time;
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form method="post" action="/Printing/setDate/" id="updateDateForm">
...
<div class="box box-default">
    ...
    <div class="box-header">
        ...
        <h3 class="box-title"><i class="fa fa-print"></i> <?= __('Orders bij drukkerij') ?></h3>
    </div>
    ...
    <div class="box-body">
            ...
            <ul class="nav nav-tabs">
                ...
                <?php if($this->request->session()->read("adminlogin")["supplier_nrint"] == ""){ ?>
                    <div class="pull-right">
                        <form method="post" action="/Printing/setSession/">
                            <select name="printer_nrint" onchange="this.form.submit()" required="required">
            ...
            <div class="tab-content">
                <?php if($session == ""){ ?>
                    <div class="tab-pane active" id="PRINTING">

                        <?php if($aOrders->count() == 0){ ?>
                            <br/><p class="alert alert-warning"><?= __('Geen orders in de hoofdstatus') ?> <b><?= __('Orders bij drukkerij') ?></b></p>
                        <?php }else{ ?>
                            <?php $total = 0; ?>
                                    ...
                                    <?php foreach($aOrders as $aOrder){ ?>
                                        <?php if($aOrder->HdIntPrplGeneral->priority == $prio){ ?>
                                            ...
                                            <tr class="<?= ($prio == 1) ? 'warning' : '' ?>" data-touch-href="/Order/<?= $aOrder->orderrule_id ?>">
                                                ...
                                                <td <?= $aOrder->HdIntPrplPrinting->end_date->format('Ymd') < date('Ymd') ? 'class="text-danger text-bold"' : '' ?>><?= $aOrder->HdIntPrplPrinting->end_date->format('d-m-Y') ?></td>
                                                ...
                                                <td><?= $aCompany[$aOrder->HdIntPrplPrinting->adres_nrint]->HdIntPrplCompanies->adres_alias ?></td>
                ...
                <?php foreach($aCompany as $company){ ?>
                    ...
                    <div class="tab-pane active" id="<?= $company["BKHADNRINT"] ?>">
                        <form method="post" action="/Printing/saveBatch/">
                        <input type="hidden" name="printer_nrint" value="<?= $company["BKHADNRINT"] ?>" />
                                    ...
                                    <?php foreach($aGrouped[$company["BKHADNRINT"]] as $aOrder){ ?>
                                        <?php if($aOrder->HdIntPrplGeneral->priority == $prio){ ?>
                                            ...
                                            <tr class="<?= ($prio == 1) ? 'warning' : '' ?>" data-touch-href="/Order/<?= $aOrder->orderrule_id ?>">
                                                ...
                                                <td><?= $aOrder->order_nr ?></td>
                                                <?php
                                                $enddate = !is_null($aOrder->HdIntPrplPrinting->end_date_late) ? $aOrder->HdIntPrplPrinting->end_date_late :  $aOrder->HdIntPrplPrinting->end_date;
                                                if($enddate != $aOrder->HdIntPrplPrinting->end_date){
                                                    $enddate_base =  $aOrder->HdIntPrplPrinting->end_date->format('d-m-Y');
                                                }else{
                                                    unset($enddate_base);
                                                }
                                                ...
                                                <td class="text-right"><?= number_format($aOrder->HdIntPrplGeneral->order_start_quantity,0,",",".") ?></td>
                                                <td class="text-right">
                                                    <input type="number" class="form-control input-xs text-right" name="production_quantity[]"
